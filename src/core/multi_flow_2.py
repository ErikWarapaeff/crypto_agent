...
class MultiAgentSystem:
    ...
    async def process_user_input_with_clarification(self, user_input: str) -> str:
        """
        Обрабатывает запрос пользователя с возможностью уточнения.
        ...
        if len(user_input.split()) < 5 or "?" not in user_input:
            # Если запрос слишком короткий или не содержит вопрос, предлагаем уточнения
            suggestions = self.generate_query_suggestions(user_input)
            return f"""
            Ваш запрос может быть обработан несколькими способами. Вот предлагаемые варианты:
            
            {suggestions['suggestions']}
            
            Пожалуйста, выберите один из вариантов или уточните ваш запрос.
            """
        
        # Если запрос достаточно подробный, передаем его супервизорному агенту
...
class CryptoAgent:
    ...
    def _create_agent_graph(self):
        ...
        workflow.add_node("query_clarification", self._clarify_query)  # Новая нода для уточнения запросов
        
        # Установка точки входа
        workflow.set_entry_point("query_clarification")  # Изменяем точку входа на ноду уточнения
    ...
    async def _clarify_query(self, state: MessagesState):
        """Уточняет запрос пользователя и предлагает комбинации запросов."""
        messages = state["messages"]
        last_message = messages[-1] if messages else None
        
        # Если нет сообщений или последнее сообщение не от пользователя, просто передаем управление дальше
        if not last_message or not hasattr(last_message, "content") or last_message.role != "user":
            return {"messages": messages}
        
        # Получаем список доступных агентов и инструментов
        available_agents = {
            "MARKET_ANALYST": "анализирует рыночные данные, цены и тренды",
            "TECHNICAL_ANALYST": "проводит технический анализ графиков и исторических данных",
            "NEWS_RESEARCHER": "исследует новости, твиты и события",
            "TRADER": "выполняет торговые операции и анализирует их",
            "PROTOCOL_ANALYST": "анализирует протоколы, пулы ликвидности и холдеры"
        }
        
        # Создаем промпт для модели, которая будет уточнять запрос
        clarification_prompt = f"""
        Проанализируй запрос пользователя и определи, нужно ли его уточнить.
        
        В нашей системе доступны следующие специализированные агенты:
        {', '.join([f"{agent} ({desc})" for agent, desc in available_agents.items()])}
        
        Если запрос пользователя неясен или может быть обработан несколькими агентами,
        предложи пользователю варианты уточнения запроса или комбинации агентов,
        которые могут обработать его запрос наиболее эффективно.
        
        Если запрос ясен и конкретен, просто передай его дальше без изменений.
        """
        
        # Создаем новое системное сообщение с промптом уточнения
        clarification_system_message = {"role": "system", "content": clarification_prompt}
        
        # Формируем сообщения для модели
        clarification_messages = [clarification_system_message, last_message]
        
        # Вызываем модель для уточнения запроса
        clarification_response = await self.llm.ainvoke(clarification_messages)
        
        # Добавляем ответ модели в сообщения
        updated_messages = messages.copy()
        updated_messages.append(clarification_response)
        
        return {"messages": updated_messages}
    
    def _should_clarify(self, state: MessagesState) -> Literal["agent", "end"]:
        """Определяет, нужно ли уточнять запрос или передать его агенту."""
        messages = state["messages"]
        
        # Если последнее сообщение от ассистента содержит вопрос, то продолжаем диалог
        if messages and len(messages) >= 2:
            last_message = messages[-1]
            if hasattr(last_message, "role") and last_message.role == "assistant":
                # Проверяем, содержит ли сообщение вопрос или предложение уточнений
                content = last_message.content if hasattr(last_message, "content") else ""
                if "?" in content or "Выберите" in content or "Уточните" in content:
                    # Ждем ответа пользователя на уточняющий вопрос
                    return "end"
        
        # В противном случае переходим к обработке запроса агентом
        return "agent"
    
    def generate_query_suggestions(self, user_input: str) -> Dict[str, Any]:
        """
        Генерирует предложения по комбинации запросов и агентов на основе ввода пользователя.
        
        Args:
            user_input: Исходный запрос пользователя
            
        Returns:
            Словарь с предложениями и комбинациями агентов
        """
        # Получаем список всех доступных агентов
        available_agents = {
            agent_id: agent.role 
        ...
        for agent_id, agent in self.agents.items():
            if agent.role != AgentRole.SUPERVISOR:
                ...
                available_tools[agent_id] = tools
        
        # Формируем запрос для супервизорного агента
        suggestion_prompt = f"""
        На основе запроса пользователя "{user_input}" предложи оптимальные комбинации 
        запросов и агентов, которые могут обработать этот запрос наиболее эффективно.
        
        Доступные агенты:
        {available_agents}
        
        Каждый агент имеет доступ к следующим инструментам:
        {available_tools}
        
        Предложи не более 3 вариантов комбинаций запросов и агентов.
        """
        
        # Используем супервизорного агента для генерации предложений
        supervisor = self.agents["supervisor"]
        result = asyncio.run(supervisor.process_user_input(suggestion_prompt))
